...
- [x] Phase 10 (slice 1) — bit-level frame inspection (§29): `InspectFrame` → named SOF..CRC regions + full wire bits over WS, "Show bit layout" in the frame inspector (waveform rendering deferred)
- [x] Phase 10 (slice 2) — waveform strip in the inspector: per-bit SOF..EOF cells with SOF/stuffed-region/fixed-tail bands (pure band math, harness-verified; zoom/pan/cursors deferred)
...
- [x] Phase 9 (slice 4) — node disable/enable (§35): runtime-only `DisableNode`/`EnableNode` over WS, `Status.disabled` + header chip + properties toggle, reset re-enables (bus-off-style loud failures preserved)
- [x] Observability slice — `canlab replay`: deterministic headless re-run of `--export-json` traces (bit-identical logs, verified by replay-of-replay diff) plus PCAP import (`DLT_CAN_SOCKETCAN`, magic-detected, `--as` attributes senders; TS-export/Rust-import agreement verified live)
