...
- **CAN Analyzer**: streams TX/RX frames as they happen (Time, Dir, Node,
  ...
  regions plus a per-bit waveform strip and wire/stuff counts; **Decode with DBC** decodes signals against
...
- **Fault injection**: drive one corrupted frame now (flip a wire bit,
  ...
  (`examples/faults.canlab.yaml`), authored in the Fault policies panel
  or the file. Details:
