...
firmware file (missing firmware is fatal for nodes that execute it,
i.e. `renode` backends — virtual references are skipped with a warning,
mirroring `validate`/`simulate`), and the `dbc/`/`assets/` companions
when present, preserving relative layout so the packaged project
validates and simulates from its new home. The output directory must
not exist yet.
